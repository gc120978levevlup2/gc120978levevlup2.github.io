...
const read_categories = async (name = null, on_successfull_transaction = null) => {
    categories = []
    ...
    if (on_successfull_transaction){
        if (name){
            on_successfull_transaction(categories.filter((category) => category.name === name))
        }else{
...
const read_categories2 = async (id = null, on_successfull_transaction = null) => {
    categories = []
    const {data,error} = await supa
                                .from("categories")
                                .select('*')
    ...
    if (on_successfull_transaction){
        if (id){
            on_successfull_transaction(categories.filter((category) => category.id === id))
        }else{
            on_successfull_transaction(categories)
        }
    }
...
const create_category = ( category , on_successfull_transaction = null, on_failed_transaction) => {

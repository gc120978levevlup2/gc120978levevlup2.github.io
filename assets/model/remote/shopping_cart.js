...
const read_cart_items = async (id = null, on_successfull_transaction = null) => {
    items = []
    const {data,error} = await supa
                                .from("shopping_cart")
                                .select('*')
...
const read_cart_items_fr_seller = async (seller = null, on_successfull_transaction = null) => {
    items = []
    const {data,error} = await supa
                                .from("shopping_cart")
                                .select('*, items(*), users(*)')
...
const read_cart_items_fr_user = async (user = null, on_successfull_transaction = null) => {
    items = []
    const {data,error} = await supa
                                .from("shopping_cart")
    ...
    if (on_successfull_transaction){
        if (user){
            on_successfull_transaction(items.filter((item) => item.email1 === user.email1))
        }else{
            on_successfull_transaction(items)
        }
    }

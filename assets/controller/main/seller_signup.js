...
// view
import { seller_signup_view   } from "/assets/view/js/main/seller_signup_view.js"
...
import {
    seller_new_form_submit,
                              } from "/assets/controller/submit/seller_new_form_submit.js"


seller_signup_view()
showModal('sellersignUpModal')
document.getElementById('exampleModalLabel').innerHTML =  /*html*/`<i class="fa fa-lock fa-lg" aria-hidden="true"></i> Seller Information Form Data`;
...
seller_new_form_submit(() => {
    ...
    read_sellers(seller.email1, found_matching_sellers => {
        if (found_matching_sellers.length === 0){
            create_seller(seller, sellers => {
                if (sellers.length === 1) {
                    alert(`Successfully registered seller, ${sellers[0].email1}: ${sellers[0].name}.`)
                    window.location = "/assets/view/pages/login.html"
                }
            })
        }else{
            alert(`Failed to registered seller, ${seller.email1} account already used by another seller.`)
            window.location = "/assets/view/pages/seller_signup.html"
        }
    })
})

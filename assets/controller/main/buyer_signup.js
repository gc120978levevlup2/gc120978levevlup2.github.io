...
import { read_users, create_user } from "/assets/model/remote/users.js"
...
import { buyer_new_form_submit } from "/assets/controller/submit/buyer_new_form_submit.js"

buyer_signup_view()
showModal("sellersignUpModal")
document.getElementById(
	"exampleModalLabel"
).innerHTML = /*html*/ `<i class="fa fa-lock fa-lg" aria-hidden="true"></i> User Information Form Data`
...
buyer_new_form_submit(
	() => {},
	(user) => {
		read_users(user.email1, (found_matching_users) => {
			if (found_matching_users.length === 0) {
				create_user(user, (users) => {
					//console.log(users.length)
					if (users.length === 1) {
						alert(
							`Successfully registered user, ${users[0].email1}: ${users[0].name}.`
						)
				...
				alert(
					`Failed to registered user, ${user.email1} account already used by another user`
				)
				window.history.back()
			}
		})

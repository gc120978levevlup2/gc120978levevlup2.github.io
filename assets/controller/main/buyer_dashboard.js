    ...
    let html = items.map((item, i) => /*html*/`
                    <div class="col mb-3" style="color: gray" >
                        <div class="card mb-3 border-1 p-0 position-relative shadow mx-auto" style="width:250px;height:600px">
                            ...
                            <div class="card-body" align="center" style="padding-top:1px">
                                <a class="desc-selection" href="/assets/view/pages/search.html?search=${item.items.id}">
                                    <h4 class="d-flex align-middle desc-selection" style="height:60px;overflow-y:hidden;margin-bottom:1px;font-size:13px">${item.items.description}</h4>
                                </a>
                                <p class="d-flex align-middle" style="height:45px;overflow-y:hidden;margin-bottom:0;font-size:10px">${item.items.specs}</p>
                                ...
                                <div align="center" class="d-flex justify-content-center" style="height:25px;padding:0">
                                    <p class="text-capitalize" style="font-size:15px">${item.items.sellers.name}</p>
                                </div>
                                <div align="center" class="d-flex justify-content-center" style="height:15px;padding:0">
                                    <p style="font-size:10px">${item.items.sellers.email1}</p>
                                </div>
                                <div align="center" class="d-flex justify-content-center" style="height:15px;padding:0">
                                    <p style="font-size:10px">${item.items.sellers.contact}</p>
                                </div>
                                ...
                                <hr>
                                <div align="center" class="d-flex justify-content-center" style="height:25px;padding:0">
                                    <span id="see-mod-${item.id}" class="badge bg-primary categories-selection" style="font-size:12px">${item.status_update}</span>
                                </div>
                                <br>
                                <div class="row g-1">
                                    <div class="col-6">
                                    ...
                                    <div class="col-6">
                                        <button 
                                           ...
                                           data-bs-toggle="modal" data-bs-target="#static-review-${item.id}"
                                         >
                                            <i class="bi bi-pen"></i>
                                            Review
                                        </button>
                        ...
                        <div class="modal fade" id="staticBackdrop2-${item.id}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                            <div class="modal-dialog modal-lg modal-dialog-centered  modal-dialog-scrollable">
                                <div class="modal-content">
                                    <div class="modal-header bg-danger">
                                        <h5 class="modal-title text-white" id="staticBackdropLabel">Are you sure you want to CANCEL ${item.items.description} ORDER?</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                            <img
                                    ...
                                    <div class="modal-footer" id="del-del-${item.id}">
                                        <button  class="btn btn-outline-danger " data-bs-dismiss="modal">YES</button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <!-- Modal -->
                        <form>
                            <div class="modal fade" id="static-review-${item.id}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="static-review-title-${item.id}" aria-hidden="true">
                                <div class="modal-dialog modal-lg modal-dialog-centered  modal-dialog-scrollable">
                                    <div class="modal-content">
                                        <div class="modal-header bg-primary">
                                            <h6 class="modal-title text-white" id="static-review-title-${item.id}">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="row">
                                                <div id="image-viewer-${item.id}" class="col-lg-4 p-2">
                                                    <h6>[No Images Uploaded]</h6>
                                                </div>
                                                <div class="col-lg-8 p-2 border-start">
                                                    <input type="text" name="item_id" value="${item.items.id}" class="form-control d-none">
                                                    <input type="text" name="user_is_a_buyer" value="${global_user_buyer}" class="form-control d-none">
                                                    <input type="text" name="user_email1" value="${global_user.email1}" class="form-control d-none">
                                                    <input type="text" name="images" value="[]" class="form-control d-none" id="image-${item.id}">
                                                    <input type="text" name="date" value="${(new Date(Date.now())).toString()}" class="form-control d-none">
                                                    ...
                                                    <div class="form-floating mb-3">
                                                        <input type="file" class="form-control" id="review-image-${item.id}" placeholder="try"></input>
                                        ...
                                        <div class="modal-footer" id="review-btn-${item.id}">
...
read_user( user => {
    read_cart_items_fr_user( user, items => {
        ...
        $('#buyer-shipment-list').html( getProductListInnerHTMLof_users_deliveries(upcoming_shipments) )
                ...
                img2.addEventListener("change", (e) => {
                    handleImageFileUpload(e, 400, resized_file => {
                        ...
                        save_image(resized_file, makeid, file_path => {
                            let images = JSON.parse($(`#image-${d_item.id}`).val())
...
function getProductImage(item_id){
    ...
    document.getElementById(`image-viewer-${item_id}`).innerHTML = 
        images.map ( (value,i) => (/*html*/`
            <div class="mx-auto w-75">
                <img
                    width="100%"

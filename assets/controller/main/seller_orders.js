                    ...
                    <div class="col mb-3" style="color: gray" >
                        <div class="card mb-3 border-1 p-0 position-relative shadow mx-auto" style="width:250px;height:600px">
                            ...
                            <div class="card-body" align="center" style="padding-top:1px">
                                ...
                                <p class="" style="height:45px;overflow-y:hidden;margin-bottom:0;font-size:10px">${
									item.items.specs
								}</p>
                            ...
                            <div class="modal-dialog modal-lg modal-dialog-centered  modal-dialog-scrollable">
                                <div class="modal-content">
                                    <div class="modal-header bg-danger">
                                        <h5 class="modal-title text-white" id="staticBackdropLabel">Are you sure you want to DELETE ${
											item.items.description
										} Product?</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
...
seller_product_form_submit2(
	...
	(item) => {
		update_item(item, (items) => {
			// on successfully saved item
...
read_user((user) => {
	...
	read_cart_items_fr_seller(user, (items) => {
		...
		$("#seller-orders-list").html(getProductListInnerHTMLof(products))
		...
		for (let item of products) {
			...
			del_btn2.addEventListener("click", (e) => {
				...
				read_cart_items(product_id, (cart_items) => {
					...
					read_cart_items(product_id, (item) => {
						...
						read_items(value.item_id, (items2) => {
							...
							for (let key in items2[0]) {
								if (key !== "on_sale" && key !== "on_stop_sell")
	...
	img.addEventListener("change", (e) => {
		handleImageFileUpload(e, 400, (resized_file) => {
			...
			save_image(resized_file, makeid, (file_path) => {
				...
				document
					.getElementById("category-image-div")
...
function getProductImage() {
	...
	document.getElementById("product-image-div").innerHTML = images
		.map(
			(value, i) => /*html*/ `
            <div class="mx-auto w-75">
                <img
...
admin_new_cat_form_submit(
	...
	(category) => {
		create_category(
			category,
			(categories) => {
				// on successfully saved category
				//sconsole.log(categories)
				window.location = "/assets/view/pages/seller_product.html"
			},
			() => {
